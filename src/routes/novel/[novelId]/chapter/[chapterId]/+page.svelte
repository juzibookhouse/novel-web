<script lang="ts">
  ...
  $: chapterReadingStatus =
    data.chapter?.is_free || data.chapter?.novels?.is_free;
...
<div class="max-w-4xl mx-auto relative">
  <div
    class="fixed top-1/2 right-[calc(50vw-520px)] transform -translate-y-1/2 z-10 bg-white p-3 rounded shadow-xl"
  >
    <a
  ...
  <div class="bg-[#f2f2f2] backdrop-blur-sm rounded-lg shadow-xl">
    ...
    <div class="pt-8">
      ...
      {#if chapter.published && chapter.novels.published}
        ...
        {:else if !$user}
          <ChapterLoginMsg {novelId} chapterId={chapter.id} />
        {:else if !$user?.isMembership}
          <ChapterMemberSubscriptionMsg
            handleMembershipModal={() => (showMembershipModal = true)}
          />
        {/if}
      {:else if chapter.novels.user_id === $user?.id}
        <!-- 作者可以看到未发布的章节 -->
        <div
          class="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 mb-4 mx-4"
        >
    ...
    <ChapterPagination {novelId} {prevChapterId} {nextChapterId} />

    <ChapterGifts chapterId={chapter.id} {novelId} />

    <ChapterComments chapterId={chapter.id} {novelId} {toggleKeydownEvent} />
  </div>
</div>
