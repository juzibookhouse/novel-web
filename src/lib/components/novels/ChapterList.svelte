<script lang="ts">
  export let novel;

  // Filter only published chapters
  $: publishedChapters = novel.chapters.filter(chapter => chapter.published);
</script>

<!-- Chapter List -->
<div class="p-8">
  <h2 class="font-['Ma_Shan_Zheng'] text-2xl text-primary mb-6">目录</h2>
  <div class="grid grid-cols-2 gap-4">
    {#each publishedChapters as chapter}
      <a
        href={`/novel/${novel.id}/chapter/${chapter.id}`}
        class="group p-4 rounded-lg border-2 border-gray-400 hover:border-red-300 hover:bg-red-50 transition-all duration-200 relative"
      >
        <h3
          class="text-lg text-gray-900 group-hover:text-primary transition-colors duration-200"
        >
          {chapter.title}
        </h3>
        {#if novel.is_free || chapter.is_free}
          <span class="absolute top-2 right-2 bg-green-100 text-green-800 text-xs px-2 py-1 rounded-full">
            免费
          </span>
        {:else}
          <span class="absolute top-2 right-2 bg-red-100 text-primary text-xs px-2 py-1 rounded-full">
            会员
          </span>
        {/if}
      </a>
    {/each}
  </div>
</div>
